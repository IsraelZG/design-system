...
import globalColor from '../../../design-system/tokens/global/color.json';
import { defaultLightTheme, defaultDarkTheme, type ThemeJSON } from '@ds/index';
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        {activeTab === 'presets' ? (
          ...
          <div className="space-y-2">
            ...
            >
              <div className="space-y-3 pt-2">
                {currentEditableTheme.theme.intent?.primary &&
                  Object.keys(currentEditableTheme.theme.intent.primary).map(key => {
                    const token = currentEditableTheme.theme.intent.primary[key];
                    if (token.type !== 'color') return null;
                    return (
                      <ColorInputRow
                        key={key}
            ...
            >
              <div className="space-y-3 pt-2">
                {currentEditableTheme.theme.intent?.accent &&
                  Object.keys(currentEditableTheme.theme.intent.accent).map(key => {
                    const token = currentEditableTheme.theme.intent.accent[key];
                    if (token.type !== 'color') return null;
                    return (
                      <ColorInputRow
                        key={key}
