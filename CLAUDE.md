...
If nothing matches and the user's request is genuinely novel, **do not silently invent a component**. Follow the new-component flow:

1. First, check whether the case can be satisfied by adding a `commonPatterns` entry to an existing component. Most "new" UI is composition.
2. If a genuinely new component is needed, read `docs/AUTHORING.md` for the full 12-step authoring sequence and skeleton. The `.claude/skills/design-system-component-author/` skill auto-loads when the trigger matches.
3. Ask the user before scaffolding: *"None of the existing components cover X. I can either (a) extend `<closest match>` with a new pattern, or (b) create a new component `<Name>` following the authoring spec. Which?"*

Never create a component without surfacing this choice.
...
| `docs/AUTHORING.md` | Full step-by-step for creating a new component. Read before scaffolding. |
| `.claude/skills/design-system-component-author/` | Auto-loading skill that triggers when the user asks for a new component. |
